...
repo_url="${ENGINE_INPUT_PRODUCERS_GIT_REPO:-https://github.com/omenien/omena-engine-input-producers.git}"
repo_ref="${ENGINE_INPUT_PRODUCERS_GIT_REF:-0b7cd56}"
temp_dir="$(mktemp -d)"
...
mod tests {
    ...
    #[test]
    fn consumes_remote_input_producers_repo_via_git_dependency() -> Result<(), String> {
...
RUSTUP_TOOLCHAIN=stable cargo test --manifest-path "${temp_dir}/Cargo.toml" -p engine-input-producers-git-consumer-check
RUSTUP_TOOLCHAIN=stable cargo check --manifest-path "${temp_dir}/Cargo.toml" -p engine-input-producers-git-consumer-check
RUSTUP_TOOLCHAIN=stable cargo clippy --manifest-path "${temp_dir}/Cargo.toml" -p engine-input-producers-git-consumer-check --all-targets --all-features -- -D warnings
