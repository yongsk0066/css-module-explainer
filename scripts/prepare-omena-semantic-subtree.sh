...
branch="${1:-split/omena-semantic}"

if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  cat <<'EOF'
Usage:
  ./scripts/prepare-omena-semantic-subtree.sh [branch-name]

Default branch:
  split/omena-semantic

This creates a local history-preserving subtree branch for:
  rust/crates/omena-semantic

Equivalent raw command:
  git subtree split --prefix=rust/crates/omena-semantic -b <branch-name>
EOF
  exit 0
fi
