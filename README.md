...
- `.github/workflows/checker-release-gate-shadow.yml` runs that same shadow soak on every `master` push without changing the stable release gate, builds the repo artifacts plus `server/engine-core-ts` dist needed by `rust-gate-evidence`, and executes the release bundle components, checker entrance, and promotion evidence as separate steps so failures are attributable from the workflow UI
