...
- `.github/workflows/checker-release-gate-shadow.yml` runs that same shadow soak on every `master` push without changing the stable release gate, and it now executes the release bundle components, checker entrance, and promotion evidence as separate steps so failures are attributable from the workflow UI
