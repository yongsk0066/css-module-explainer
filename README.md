...
- Current `4.1.2` framing is the Phase 2 swap runtime-hardening milestone on top of the Rust-backed semantic core GA baseline:
  ...
  - `cssModuleExplainer.lspServerRuntime=omena-lsp-server` is the opt-in Rust stdio LSP runtime path; default runtime stays on the mature Node server while the Rust path carries provider parity coverage for style providers, source selector providers, source/style diagnostics, quick fixes, and watched-file style index updates
