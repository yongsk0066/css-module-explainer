...
use omena_semantic::{summarize_style_semantic_graph, summarize_theory_observation_harness};
use serde::Deserialize;

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct TheoryObservationInput {
    style_path: String,
    style_source: String,
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let input: TheoryObservationInput = serde_json::from_str(&input_json)?;
    ...
    let summary = summarize_theory_observation_harness(&graph);
